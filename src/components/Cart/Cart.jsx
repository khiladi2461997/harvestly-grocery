...
import { addProduct, subtractProduct, removeProduct } from '../../reducers/cartSlice'
...
const Cart = () => {
    ...
    const fetchCartItems = async () => {
      try {
        ...
        if (loggedInUsers.length > 0) {
          ...
          const Productresponse = await axios.get(`https://harvestlyy.onrender.com/api/app/cart/${userId}`);
  ...
  const removeFromCart = async ( userId , productId) => {
    try {
      ...
      await axios.delete(`https://harvestlyy.onrender.com/api/app/cart/${userId}/${productId}/remove`);
        ...
        {cartItems.map((item, index) => (
          <div key={index} className="border-b border-neutral-200 py-4 px-6">
            ...
            <div className="mt-4">
              ...
              <img  onClick={() => removeFromCart(item.userId , item.productId)} className="w-6 h-6 ml-4 relative" src={Close} alt="Close Icon" />
            </div>
          </div>
        ))}
      </div>
